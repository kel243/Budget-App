...
function setup() {
  ...
  document
    ...
    .addEventListener("click", addCategory, true);
  document
    .getElementById("addPurchase")
    .addEventListener("click", addPurchase, true);
...
function addPurchase() {
  ...
  var data;
  data =
    "name=" + newPurchase + "&cost=" + cost + "&cat=" + cat + "&date=" + date;
  window.clearTimeout(timeoutID);
...
function calculate() {
  ...
  cats.forEach(function(e) {
    ...
    if (e.remaining > 0) {
      document.querySelectorAll(".remaining")[i++].innerHTML =
        "$" + e.remaining;
    } else {
